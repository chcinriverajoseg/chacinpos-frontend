        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 flex justify-between items-center">
            ...
            <button onClick={handleAbrirCaja}
              className="flex items-center gap-2 px-6 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg text-sm transition-colors">
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {discrepancias.length > 0 && (
        <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-xl mb-5 text-red-700 text-sm">
          ...
          <span>Alerta: billetes de {discrepancias.map(d => labelDenom(d)).join(', ')} tienen más salidas que entradas. Verificar caja.</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label:'Ventas del día',   value: clp(totalVentas),    sub:`${ventas.length} ventas`, color:'text-green-600' },
          { label:'Entradas',         value: clp(totalEntradas),  sub:'efectivo recibido',       color:'text-green-600' },
          { label:'Efectivo en caja', value: clp(efectivoActual), sub:'estimado actual',         color:'text-green-700' },
        ].map(s => (
