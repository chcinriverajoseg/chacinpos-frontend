...
export default function Dashboard() {
  ...
  const STATS = [
    { label:'Ventas hoy',    value: clp(totalHoy),       sub:`${ventasHoy.length} transacciones`, icon: TrendingUp,  color:'text-green-600',  bg:'bg-green-50'  },
    { label:'Transacciones', value: ventasHoy.length,    sub:'documentos emitidos',               icon: ShoppingCart,color:'text-blue-600',   bg:'bg-blue-50'   },
    { label:'Stock bajo',    value: stockBajos.length,   sub:`${sinStock.length} sin stock`,      icon: Package,     color:'text-amber-600',  bg:'bg-amber-50'  },
    { label:'Estado caja',   value: cajaAbierta ? 'Abierta' : 'Cerrada', sub:'hoy',              icon: DollarSign,  color: cajaAbierta ? 'text-green-600' : 'text-red-600', bg: cajaAbierta ? 'bg-green-50' : 'bg-red-50' },
  ]
          ...
          <div className="space-y-2">
            {[
              ...
              <button key={a.to} onClick={() => navigate(a.to)}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${a.primary ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}>
                <a.icon size={16} />
                {a.label}
            ...
            <div className="space-y-2">
              {sinStock.slice(0,3).map(p => (
                <div key={p.id} className="flex items-center gap-2 px-3 py-2 bg-red-50 rounded-lg">
                  ...
                  <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-medium">Sin stock</span>
                </div>
              ))}
              {stockBajos.slice(0,3).map(p => (
                <div key={p.id} className="flex items-center gap-2 px-3 py-2 bg-amber-50 rounded-lg">
                  <span className="text-base">{p.emoji}</span>
                  <span className="text-sm text-amber-800 flex-1 truncate">{p.nombre}</span>
                  <span className="text-xs bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-medium">{p.stock} uds</span>
                </div>
              ))}
            </div>
          )}
              ...
              {ventasHoy.slice(-5).reverse().map(v => (
                <div key={v.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{v.tipo === 'factura' ? '📄 Factura' : '🧾 Boleta'}</p>
                  </div>
                  <span className="text-sm font-semibold text-green-600">{clp(v.total)}</span>
                </div>
              ))}
            </div>
          )}
